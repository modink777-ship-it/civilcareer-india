  ...
  function boot() {
    ...
    if (form && !form.dataset.ccNl) {
      ...
      document.addEventListener('submit', function (e) {
        ...
        if (q.includes(',')) return; /* explicit multi-role / multi-city entry → compound search */
        const p = parseQuery(q);
        ...
        if (window.ccFacetState) {
          /* NEW filter bar: carry parsed intent into the one-line bar and run
             the server-side paginated explorer. Bar never throws when a facet
             id is missing — every write is guarded. */
          e.preventDefault();
          e.stopPropagation();
          const st = window.ccFacetState.private;
          if (p.role) st.role = p.role;
          if (p.city) st.location = p.city;
          if (p.state) st.state = p.state;
          if (p.exp !== null) st.experience = p.exp === 0 ? 'Fresher' : String(p.exp);
          if (p.freshness) st.posted = String(p.freshness);
          if (typeof renderPrivateFilterBar === 'function') renderPrivateFilterBar();
          if (typeof navigate === 'function') navigate('private'); else location.href = '/private-jobs';
