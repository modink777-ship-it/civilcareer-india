...
function renderExams(code=''){const a=exams.filter(isCivilExam).filter(x=>!code||String(x.code||'').toUpperCase().includes(code.toUpperCase()) || String(x.title_en||'').toUpperCase().includes(code.toUpperCase()));$('examCards').innerHTML=a.length?a.map(examCard).join(''):empty('Exam updates are being added','Only civil-engineering and civil-recruitment examination updates are shown here.');bindCards()}
function renderMaterials(cat=''){const a=materials.filter(isCivilMaterial).filter(m=>!cat||String(m.category||'').toLowerCase().includes(String(cat).toLowerCase())||String(m.exam_code||'').toLowerCase().includes(String(cat).toLowerCase()));$('materialCards').innerHTML=a.length?a.map(materialCard).join(''):empty('Resources are being added','Only civil-engineering study materials are shown here. General competitive-exam material is excluded.');bindCards()}
