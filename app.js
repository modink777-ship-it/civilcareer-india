...
async function runDiscovery(){
  ...
  try{
    const data=await api('/api/discover',{method:'POST',key:adminKey,body:JSON.stringify({})});
    if(st){st.textContent=data.message||'Discovery finished.';st.className='discovery-status success';}
    if(box)box.innerHTML=`<div class="discovery-summary"><b>${Number(data.added||0)}</b> new drafts added <span>·</span> ${Number(data.skipped||0)} skipped</div><div class="discovery-source-list">${(data.feeds||[]).map(f=>`<span>${esc(f.name)}: ${f.error?'error':Number(f.items||0)+' items'}</span>`).join('')}</div><p>Open <strong>Jobs</strong> to review, edit and publish the drafts.</p>`;
    await loadAdmin();
