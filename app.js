...
const metas={home:['CivilCareer — India’s Civil Engineering Career Platform','India’s dedicated platform for civil engineering jobs, MNC careers, civil-focused government recruitment, exams and study resources.'],private:['Civil Engineering Jobs | CivilCareer','Civil engineering jobs across India in construction, infrastructure, consulting, real estate, EPC and engineering organizations.'],government:['Government Civil Jobs in India | CivilCareer','Civil-focused Central and State government recruitment for Civil Engineers, diploma holders and related technical candidates.'],exams:['Civil Engineering Exams | CivilCareer','GATE, ESE, SSC JE, RRB JE, AE, JE and relevant civil engineering examination updates across India.'],materials:['Free Civil Engineering & Exam Study Materials | CivilCareer','Free organized civil engineering and relevant competitive-exam resources.'],post:['Post a Civil Engineering Job | CivilCareer','Submit a legitimate civil engineering job for moderation.'],resource:['Submit a Study Resource | CivilCareer','Submit a study resource you own or have permission to distribute.'],report:['Report a Problem | CivilCareer','Privately report suspicious, incorrect, expired or copyrighted content.'],about:['About CivilCareer','Learn about CivilCareer’s safety, accuracy and official-source principles.'],search:['Search CivilCareer','Search civil engineering jobs, government civil recruitment, exams and resources.'],admin:['CivilCareer Admin','Protected CivilCareer administration.']};function setMeta(){const m=metas[route]||metas.home;document.title=m[0];document.querySelector('meta[name="description"]').content=m[1]}
...
function isCivilGovernmentJob(j){
  const t=((j.role||'')+' '+(j.discipline||'')+' '+(j.qualification||'')+' '+(j.description||'')+' '+(j.company||'')+' '+(j.recruitment_authority||'')).toLowerCase();
  const keys=['civil','structural','construction','building','road','highway','bridge','metro','rail','water','irrigation','hydraulic','environment','survey','geotechnical','quantity survey','planning engineer','junior engineer','je civil','assistant engineer','aee','civil engineering','pwd','cpwd','nhai','bro','mes','water resources'];
  return keys.some(k=>t.includes(k));
}
function isMncOrEpcJob(j){
  const t=((j.company||'')+' '+(j.description||'')+' '+(j.employer_type||'')+' '+(j.company_type||'')).toLowerCase();
  return ['mnc','epc','engineering consulting','multinational','infrastructure group','global engineering','international engineering'].some(k=>t.includes(k));
}
function renderGovernment(){
  let a=jobs.filter(j=>['Government','Public Sector'].includes(j.sector) && isCivilGovernmentJob(j));
  if($('govState')&&!$('govState').options.length) $('govState').innerHTML='<option value="">All India</option>'+INDIA_STATES.map(s=>`<option>${s}</option>`).join('');
  const state=($('govState')&&$('govState').value||'').toLowerCase();
  ...
  a=a.filter(j=>!state||String(j.state||j.location||'').toLowerCase().includes(state))
     .filter(j=>!loc||String(j.location||'').toLowerCase().includes(loc))
     .filter(j=>!qual||String(j.qualification||'').toLowerCase().includes(qual))
  ...
  if($('governmentCount'))$('governmentCount').textContent=`${a.length} civil government opportunit${a.length===1?'y':'ies'}`;
  $('governmentJobs').innerHTML=a.length?a.map(x=>jobCard(x,true)).join(''):empty('No matching civil government recruitment','Civil-focused Central and State government opportunities will appear here after verification.');
  bindCards();
}

function renderExams(code=''){const a=exams.filter(x=>!code||x.code.toUpperCase().includes(code));$('examCards').innerHTML=a.length?a.map(examCard).join(''):empty('Exam updates are being added','Civil engineering examination details will appear here after source verification.');bindCards()}
function renderMaterials(cat=''){const a=materials.filter(m=>!cat||String(m.category).includes(cat)||String(m.exam_code).includes(cat));$('materialCards').innerHTML=a.length?a.map(materialCard).join(''):empty('Resources are being added','Free civil engineering and relevant competitive-exam materials will appear here as they are published.');bindCards()}
...
const sug=['Civil Engineer','Site Engineer','Planning Engineer','Quantity Surveyor','Junior Engineer','GATE Civil','ESE Civil','SSC JE Civil','RRB JE Civil','Bengaluru','Mumbai'];$('globalQuery').oninput=e=>{const q=e.target.value.toLowerCase();const a=sug.filter(x=>x.toLowerCase().includes(q)).slice(0,5);$('suggestions').innerHTML=a.map(x=>`<button type="button">${x}</button>`).join('');$('suggestions').classList.toggle('show',q.length>0&&a.length>0);$$('#suggestions button').forEach(b=>b.onclick=()=>{$('globalQuery').value=b.textContent;$('suggestions').classList.remove('show')})};$('smartSearch').onsubmit=e=>{e.preventDefault();$('suggestions').classList.remove('show');search($('globalQuery').value,$('globalLocation').value)};
['privateRole','privateExperience','privateType','privateSort'].forEach(id=>$(id).onchange=renderPrivate);['privateLocation','privateQualification'].forEach(id=>$(id).oninput=renderPrivate);['govStatus','govSort','govState','govLocation','govQualification','govEdu'].forEach(id=>{const el=$(id);if(el)el.addEventListener('change',renderGovernment)});
