...
/* Future-ready company branding. Explicit company fields are preferred. */
const CIVILCAREER_COMPANIES=window.CIVILCAREER_COMPANIES||{};
function companyKey(name){return String(name||'').trim().toLowerCase().replace(/[^a-z0-9]+/g,' ').replace(/\s+/g,' ')}
function safeHttpUrl(value){try{const u=new URL(String(value||''));return /^https?:$/.test(u.protocol)?u.href:''}catch{return''}}
function companyBrand(j){
  const name=String(j.company||j.recruitment_authority||'Organization').trim();
  const db=CIVILCAREER_COMPANIES[companyKey(name)]||{};
  const website=safeHttpUrl(j.company_url||j.company_website||j.website_url||j.employer_url||db.website_url||'');
  const directLogo=safeHttpUrl(j.logo_url||j.company_logo||j.employer_logo||db.logo_url||'');
  let logoUrl=directLogo;
  if(!logoUrl&&website){try{logoUrl=`https://www.google.com/s2/favicons?domain=${encodeURIComponent(new URL(website).hostname.replace(/^www\./,''))}&sz=128`}catch{}}
  const initials=name.split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join('').toUpperCase()||'C';
  return {name,website,logoUrl,initials};
}
function companyIdentity(j){
  const b=companyBrand(j);
  const logo=b.logoUrl?`<img class="company-logo" src="${esc(b.logoUrl)}" alt="${esc(b.name)} logo" loading="lazy" referrerpolicy="no-referrer" onerror="this.hidden=true;this.nextElementSibling.hidden=false">`:'';
  const fallback=`<span class="company-logo-fallback" ${b.logoUrl?'hidden':''}>${esc(b.initials)}</span>`;
  const content=`<span class="company-identity">${logo}${fallback}<span class="company-name">${esc(b.name)}</span></span>`;
  return b.website?`<a class="company-link" href="${esc(b.website)}" target="_blank" rel="noopener noreferrer" title="Visit ${esc(b.name)} official website">${content}</a>`:content;
}

jobCard=function(j,gov=false){const closed=!active(j),loc=jobLocs(j).join(' · ')||j.location_display||j.location;return`<article class="job-card"><div class="card-top"><span class="pill ${closed?'closed':j.featured?'featured':'verified'}">${closed?'Expired':j.featured?'Featured':'Active'}</span><span class="verified-date">${ago(pubDate(j))}</span></div><h3>${esc(j.role)}</h3><div class="organization">${companyIdentity(j)}</div><div class="card-meta"><span>${esc(loc||'Location in source')}</span>${jobQuals(j).slice(0,2).map(x=>`<span>${esc(x)}</span>`).join('')}${jobExps(j).slice(0,1).map(x=>`<span>${esc(x)}</span>`).join('')}</div><p class="card-copy">${esc(short(j.description||'Verify details at the original source.'))}</p><div class="card-actions"><button data-job="${j.id}">View Details</button></div></article>`}
