...
const arr=(v,f='')=>{if(Array.isArray(v)){return v.length?v:(f?[f]:[])}return v?String(v).split(',').map(x=>x.trim()).filter(Boolean):(f?[f]:[])};
...
function jobLocs(j){return arr(j.locations,j.location_display||j.location||[j.city,j.district,j.state,j.country].filter(Boolean).join(', '))}function jobQuals(j){return arr(j.qualifications,j.qualification)}function jobExps(j){return arr(j.experience_ranges,j.experience_level)}function jobTypes(j){return arr(j.employment_types,j.employment_type)}
...
openJob=function(j,push=true){if(!j)return;const closed=!active(j),email=!j.application_email_private&&j.application_email,loc=jobLocs(j).join(' · ')||j.location_display||j.location||[j.city,j.district,j.state,j.country].filter(Boolean).join(', ');const qual=jobQuals(j).join(' · ')||'Not stated',exp=jobExps(j).join(' · ')||'Not stated',employment=jobTypes(j).join(' · ')||'Not stated',salary=j.salary||((j.salary_min!=null||j.salary_max!=null)?String(j.salary_min??'')+(j.salary_min!=null&&j.salary_max!=null?' - ':'')+String(j.salary_max??'')+' '+String(j.salary_currency||'INR'):'Not stated'),state=j.state||j.state_region||'Not stated',country=j.country||'India',vacancies=j.vacancy_count??j.vacancies??'Not stated',ageLimit=j.age_limit||'Not stated',fee=j.application_fee||'Not stated',start=j.application_start||'';$('jobDetailPage').innerHTML=`<article class="dedicated-card"><div class="detail-kicker">${closed?'Expired opportunity':'Verified opportunity'} · ${ago(pubDate(j))}</div><h1>${esc(j.role)}</h1><p class="detail-lead">${esc(j.company||'Organization')} · ${esc(loc||'Location in source')}</p>${closed?'<div class="expired-banner">This opportunity has expired and is retained for transparency. Do not treat it as open.</div>':''}<section class="job-overview"><div><b>Location</b><span>${esc(loc||'Not stated')}</span></div><div><b>State</b><span>${esc(state)}</span></div><div><b>Country</b><span>${esc(country)}</span></div><div><b>Qualification</b><span>${esc(qual)}</span></div><div><b>Experience</b><span>${esc(exp)}</span></div><div><b>Employment</b><span>${esc(employment)}</span></div><div><b>Salary / Pay</b><span>${esc(salary)}</span></div><div><b>Vacancies</b><span>${esc(vacancies)}</span></div><div><b>Application start</b><span>${start?date(String(start).slice(0,10)):'Not stated'}</span></div><div><b>Deadline</b><span>${j.deadline?date(String(j.deadline).slice(0,10)):'Not stated'}</span></div><div><b>Age limit</b><span>${esc(ageLimit)}</span></div><div><b>Application fee</b><span>${esc(fee)}</span></div><div><b>Published</b><span>${date(String(pubDate(j)).slice(0,10))}</span></div></section><div class="detail-grid"><div class="detail full"><b>Description</b>${richText(j.description||'No description provided.')}</div>${j.responsibilities?`<div class="detail full"><b>Responsibilities</b>${richText(j.responsibilities)}</div>`:''}${j.skills?`<div class="detail full"><b>Skills</b>${richText(j.skills)}</div>`:''}</div>${email?`<div class="email-apply"><b>Apply via Email</b><span>${esc(email)}</span><button data-copy-email="${esc(email)}">Copy email</button><a href="mailto:${esc(email)}">Email Application</a></div>`:''}<div class="card-actions">${j.application_url||j.source_url?`<a href="${esc(j.application_url || j.source_url)}" target="_blank" rel="noopener">APPLY NOW ↗</a>`:''}${j.source_url?`<a href="${esc(j.source_url)}" target="_blank" rel="noopener">View original source ↗</a>`:''}</div><div class="callout">Always verify the job, deadline and application instructions at the original source. Never pay for a job.</div></article>`;activateDynamic('jobDetail',jobPath(j),push);document.title=`${j.role} — ${j.company||'CivilCareer'}`;$$('[data-copy-email]').forEach(b=>b.onclick=()=>navigator.clipboard.writeText(b.dataset.copyEmail).then(()=>toast('Email copied.')))}
