...
const oldImport=importJobLink;importJobLink=async function(){scanEmails();await oldImport()};$('importJobText').addEventListener('input',scanEmails);$('useDetectedEmail').onclick=()=>{if($('detectedEmailSelect').value)$('importApplicationEmail').value=$('detectedEmailSelect').value};$('importJobBtn').onclick=async()=>{const url=$('importJobUrl').value.trim(),text=$('importJobText').value.trim();if(!url&&!text)return importStatus('importJobStatus','Paste a URL or source text.','error');const b=$('importJobBtn');b.disabled=true;b.textContent='Extracting…';importStatus('importJobStatus',url&&!text?'Reading the public vacancy page…':'Analyzing the vacancy text…');try{const data=await api('/api/extract',{method:'POST',key:adminKey,body:JSON.stringify({url,text})}),x=data.extracted||data.job||{};x.application_email=$('importApplicationEmail').value||x.application_email;x.application_emails=emailList(text);x.source_url=x.source_url||url;const mode=data.source?.includes('groq')||data.source?.includes('gemini')?'AI-assisted extraction complete.':data.source?.includes('jina')||data.source?.includes('google-reader')?'Free public reader fetched the page.':'Structured/free fallback extraction complete.';importStatus('importJobStatus',data.warning||`${mode} Verify every field before saving.`,'success');jobEditor(x)}catch(e){importStatus('importJobStatus',e.message,'error')}finally{b.disabled=false;b.textContent='Extract & fill fields'}};
