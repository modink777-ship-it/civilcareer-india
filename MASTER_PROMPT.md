...
NO React, Vue, Angular, Next.js or any JS framework — ever
NO TypeScript — plain .js files only
NO build step — no Vite, Webpack, Parcel, Rollup
NO new npm packages on client-side JS
NO `app/` folder or `next.config.ts` — these break Vercel static serving
NEVER expose SUPABASE_SERVICE_ROLE_KEY in client-side code
NEVER publish a job or material automatically — everything goes through admin review first
All new API handlers go in `/_api/` NOT in `/api/` directly
Every new handler MUST be registered in `api/[[...path]].js` dispatch table
Admin auth uses `x-owner-key` header checked against `process.env.OWNER_KEY`
---
TECH STACK
Layer	Technology
...
Add `<section class="page" data-page="newpage">` in index.html
Add nav link `<a class="route" href="/new-url" data-route="newpage">`
Add rewrite in vercel.json
Handle data loading in app.js if needed
---
CSS SYSTEM
...
Steps:
Create `_api/exam-alerts.js` using the handler template above
Scrape these free sources (no API key needed):
...
Filter by civil engineering keywords
Save to `exams` table, `published: false, review_state: 'Pending Review'`
Add to dispatch table in `api/[[...path]].js`
Add cron in `vercel.json`: `{ "path": "/api/exam-alerts", "schedule": "30 0 * * 1" }`
Add "Run Now" button in `admin.html` inside `#panel-exams`
...
Steps:
Create `_api/youtube-materials.js` using handler template
Extract video ID from URL
Fetch YouTube page → parse `ytInitialPlayerResponse` JSON → get caption track URL
Fetch caption XML → clean to plain text transcript
Save to `materials` table: `published: false, review_state: 'Pending Review'`
Add to dispatch table in `api/[[...path]].js`
Add input + "Transcribe" button to `admin.html` inside `#panel-materials`
...
Do the following:
Open `lib/security.js` and read it fully
Identify what condition is causing ALL requests to be blocked (missing
env var check, IP block, misconfigured middleware, etc.)
Fix the condition so that public pages and public API endpoints
(GET /api/jobs, GET /api/exams, GET /api/materials, GET /api/health)
are accessible without any auth
Make sure the fix does NOT remove protection from admin-only endpoints
...
UPSC active exams page: https://upsc.gov.in/examinations/active-examinations
SSC homepage: https://ssc.nic.in/
