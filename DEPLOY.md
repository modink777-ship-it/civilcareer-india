...
## Final Discovery Engine

- Admin: `/admin` -> `🛰 Discovery` -> `Fetch new jobs now`
- All discoveries remain unpublished / Pending Review.
- Scheduled discovery runs from `.github/workflows/civilcareer-discovery.yml` every 6 hours and can also be run manually from GitHub Actions.
- Required existing secrets: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` (or `SUPABASE_SERVICE_KEY`).
- Optional AI secrets: `GROQ_API_KEY`, `GEMINI_API_KEY`.
- Optional Telegram notification secrets: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_ADMIN_CHAT_ID`.
- LinkedIn and Naukri are external-only search shortcuts unless an authorized feed/API is supplied.
- No database migration is required for the discovery engine.
